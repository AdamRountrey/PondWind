...
NAM_PDS_BASE_URL = "https://noaa-nam-pds.s3.amazonaws.com"
...
def build_nam_pds_url(run_at_utc: datetime, forecast_hour: int) -> str:
    file_name = build_nam_filename(run_at_utc, forecast_hour)
    return f"{NAM_PDS_BASE_URL}/nam.{run_at_utc:%Y%m%d}/{file_name}"


def _pds_has_required_fields(run_at_utc: datetime, forecast_hour: int) -> bool:
    grib_url = build_nam_pds_url(run_at_utc, forecast_hour)
        ...
        if not destination.exists():
            try:
                download_indexed_message(
                    build_nam_pds_url(run_at_utc, forecast_hour),
                    destination,
